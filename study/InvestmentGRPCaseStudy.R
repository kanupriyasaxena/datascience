...
rounds2[is.na(rounds2)]<-0
group_fund_type<-group_by(rounds2,funding_round_type)
avgInvestment<-summarize(group_fund_type,mean(raised_amount_usd))
avgInvestment1<-subset(avgInvestment,(avgInvestment$`mean(raised_amount_usd)` >5000000 & avgInvestment$`mean(raised_amount_usd)` < 15000000))
...
##master_frame_filtered<-subset(master_frame,funding_round_type == "private_equity")
master_frame_filtered<-master_frame
...
master_frame_with_sectors <- left_join(master_frame_filtered_with_primary_sector,mappings_cleanse,by = c("primary_sector" = "category_list")  )
write.csv(master_frame_with_sectors,"master_frame_with_sectors.csv")

group1<-group_by(master_frame_with_sectors,primary_sector)
summary1<-summarize(group1,sum(raised_amount_usd))

master_frame_with_sectors[is.na(master_frame_with_sectors)]<-0
subset11<-subset(master_frame_with_sectors,master_frame_with_sectors$funding_round_type=="venture"& master_frame_with_sectors$country_code=="IND")

group2<-group_by(subset11,primary_sector)
summary2<-summarize(group2,sum(raised_amount_usd),length(raised_amount_usd))


sector<-rapply(mappings_cleanse,function(x)unique(mappings_cleanse$main_sector))
sector

category<-rapply(master_frame,function(x)unique(master_frame$category_list))
category

category1<-unique(master_frame$category_list)
category1
#################################### 5.1 ################################################
...
write.csv(mappings_cleanse,"master_frame_with_sectors_agg.csv")
